      ...
      <main className="max-w-5xl w-full mx-auto px-4 py-10">
        ...
        {isAdmin && (
          <section className="mt-10">
            ...
            <p className="text-sm text-zinc-600 mt-1">
              Sản phẩm đang <span className="font-medium">pending</span> — duyệt
              để xuất hiện trên site công khai.
            </p>
            {listLoading ? (
              ...
              <ul className="mt-4 grid gap-3">
                {pendingForReview.map((p) => (
                  <li
                    key={p.id}
                    className="rounded-xl border bg-white p-4 flex flex-wrap items-start justify-between gap-4"
                  >
                    <div>
        ...
        <section className="mt-10">
          <h2 className="text-xl font-semibold">
            {isAdmin ? "Tất cả sản phẩm (theo quyền)" : "Sản phẩm của bạn"}
          </h2>
          {listLoading ? (
            <div className="text-zinc-600 mt-4">Đang tải...</div>
          ) : products.length === 0 ? (
